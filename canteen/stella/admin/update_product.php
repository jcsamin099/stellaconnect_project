...
if (isset($_POST['UpdateProduct'])) {
  ...
  if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])) {
    ...
    $postQuery = "UPDATE rpos_products SET prod_code =?, prod_name =?, prod_img =?, prod_desc =?, prod_price =?, status=? WHERE prod_id = ?";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('sssssss', $prod_code, $prod_name, $prod_img2, $prod_desc, $prod_price, $prod_status, $update);
    $postStmt->execute();
